...
Route::screen('ibkb/article/list', ArticleListScreen::class)
    ->name('platform.ibkb.article.list')
    ->breadcrumbs(fn(Trail $trail) => $trail
        ->parent('platform.index')
        ->push('База знаний по ИБ', route('platform.ibkb.article.list')));

Route::screen('ibkb/article/edit/{article}', ArticleEditScreen::class)
    ->name('platform.ibkb.article.edit')
    ->breadcrumbs(fn(Trail $trail, Article $article) => $trail
        ->parent('platform.ibkb.article.list')
        ->push('Редактирование статьи', route('platform.ibkb.article.edit', $article)));

Route::screen('ibkb/article/create', ArticleEditScreen::class)
    ->name('platform.ibkb.article.create')
    ->breadcrumbs(fn(Trail $trail) => $trail
        ->parent('platform.ibkb.article.list')
        ->push('Создание статьи', route('platform.ibkb.article.create')));